...
//
// 31-13: the phrase-login binding that used to live beside this is gone --
...
// ---- passkey management (md-4) ----------------------------------------

// PasskeyInfo is one passkey on the authenticated account. The id is a
// base64url-encoded credential id; timestamps are unix millis.
export interface PasskeyInfo {
  id: string;
  name: string; // "" when unnamed
  transports: string[];
  createdAt: number;
  lastUsedAt?: number; // absent when the passkey has never been used
}
...
function passkeyFromDTO(d: PasskeyDTO): PasskeyInfo {
  return {
    id: d.id,
    name: d.name ?? "",
...
// listPasskeys fetches the authenticated account's passkeys. Requires a
// session.
export async function listPasskeys(): Promise<PasskeyInfo[]> {
  ...
  const body = await parseResponse<ListResponse>(resp);
  return (body.passkeys ?? []).map(passkeyFromDTO);
}
...
// addPasskeyFinish completes the ceremony, persisting the new passkey
// (optionally named) against the current account, and returns it.
//
// Error codes:
//   - bad_credential            (400)
//   - parse_failed              (400)
...
//   - wrong_ceremony            (400)
//   - ceremony_validation_failed(400)
//   - ceremony_user_mismatch    (403)
//   - ceremony_expired          (410)
//   - ceremony_not_found        (404)
//   - persist_failed            (500)
export async function addPasskeyFinish(
  att: AttestationResponseJSON,
  name: string,
): Promise<PasskeyInfo> {
  const resp = await fetch("/api/auth/passkeys/add/finish", {
    method: "POST",
    credentials: "same-origin",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ credential: att, name }),
  });
  interface FinishResponse {
    passkey: PasskeyDTO;
  }
