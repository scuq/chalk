...
// `chalk-<lower semver>` siblings plus leftovers.
//
 ...
 */
export function installRoot(execPath: string, fallbackRoot: string): string | null {
  const running = dirname(execPath);
...
function untar(tar: string, archive: string, dest: string): Promise<void> {
  return new Promise((resolve, reject) => {
    const p = spawn(tar, ["-xf", archive, "-C", dest], { stdio: ["ignore", "ignore", "pipe"] });
    let err = "";
    p.stderr?.on("data", (d) => (err += String(d)));
    p.on("error", reject);
    p.on("exit", (code) => (code === 0 ? resolve() : reject(new Error(`tar exited ${code}: ${err.trim()}`))));
  });
}
...
export async function prepareUpdate(version: string, env: UpdateEnv): Promise<Prepared> {
  ...
  if (env.platform !== "win32" && env.platform !== "linux") {
    return { ok: false, stage: "unsupported", reason: `no in-place update on ${env.platform} yet` };
  }
  ...
  const root = installRoot(env.execPath, env.fallbackRoot);
  if (!root) return { ok: false, stage: "unsupported", reason: "nowhere writable to install" };

  const finalDir = join(root, versionDirName(v));
  const exe = join(finalDir, exeName(env.platform));
  if (existsSync(join(finalDir, ".ready")) && existsSync(exe)) {
  ...
  try {
    await untar(env.tar ?? "tar", download, partial);
    ...
    writeFileSync(join(finalDir, ".ready"), `${v}\n`);
  } catch (e) {
    rmSync(partial, { recursive: true, force: true });
    rmSync(finalDir, { recursive: true, force: true });
 ...
 */
export function cleanupOldVersions(root: string, currentVersion: string, runningDir: string): string[] {
