...
type Channel struct {
	...
	ExpiresAt *time.Time
	// Banner is the channel's header image and how it is framed (111-1,
	// 111-5, 111-7). Banner.AttachmentID is an attachments row, encrypted
	// under a channel key version and never linked to a message; nil means
	// no banner. Not a foreign key (attachments is partitioned, so id alone
	// is not unique) -- a dangling id resolves to no banner rather than an
	// error. The rest is presentation the server stores and never reads.
	// Populated by the read paths that select the columns: GetChannel,
	// ListChannelsForUser and the update's RETURNING; elsewhere it is the
	// zero value, which readers must treat as "no banner".
	Banner BannerLayout
}
...
type CreateChannelInput struct {
	...
	// fenced from governance frames anyway. TTL clamping against
	// CHALK_EPHEMERAL_MAX_TTL_HOURS is the handler's job; the store only
	// enforces shape.
	ExpiresAt *time.Time
}
...
func (s *Store) CreateChannel(ctx context.Context, in CreateChannelInput) (ChannelWithMembers, error) {
	...
	// 80-6: an ephemeral channel is always a plain voice room (mirrors the
	// 0050 CHECK, but as a typed error instead of a constraint violation).
	if in.ExpiresAt != nil {
		if channelType != "voice" {
			return ChannelWithMembers{}, fmt.Errorf("%w: an ephemeral channel must be a voice channel", ErrBadChannelType)
		}
		if in.IsDM {
			return ChannelWithMembers{}, fmt.Errorf("%w: a DM cannot be ephemeral", ErrBadChannelType)
		}
	}
	...
	err = s.withTx(ctx, func(tx pgx.Tx) error {
		...
		// regardless of the server-wide default -- it dies with its FKs and
		// cannot run week-long votes.
		mode := gd.Mode
		if in.ExpiresAt != nil {
			mode = "dictator"
		}
...
func (s *Store) GetChannel(ctx context.Context, channelID uuid.UUID) (Channel, error) {
	...
	err := s.Pool.QueryRow(ctx,
		`SELECT id, name, is_dm, created_by, created_at, current_key_version, rotation_pending, rotation_due_from, governance_mode, channel_type, group_name, expires_at, short_name, banner_attachment_id, banner_fit, banner_focus_x, banner_focus_y, banner_zoom, banner_height, banner_bleed
...
func (s *Store) ListChannelsForUser(ctx context.Context, userID uuid.UUID) ([]ChannelWithMembers, error) {
	rows, err := s.Pool.Query(ctx,
		`SELECT c.id, c.name, c.is_dm, c.created_by, c.created_at, c.current_key_version, c.rotation_pending, c.rotation_due_from, c.governance_mode, c.channel_type, c.group_name, c.expires_at, c.short_name, c.banner_attachment_id, c.banner_fit, c.banner_focus_x, c.banner_focus_y, c.banner_zoom, c.banner_height, c.banner_bleed,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.Name, &c.IsDM, &c.CreatedBy, &c.CreatedAt, &c.CurrentKeyVersion, &c.RotationPending, &c.RotationDueFrom, &c.GovernanceMode, &c.ChannelType, &c.GroupName, &c.ExpiresAt, &c.ShortName, &c.Banner.AttachmentID, &c.Banner.Fit, &c.Banner.FocusX, &c.Banner.FocusY, &c.Banner.Zoom, &c.Banner.Height, &c.Banner.Bleed, &lastSeq, &lastReadSeq,
