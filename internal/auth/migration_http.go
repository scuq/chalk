...
//
// The hard-cutover machinery (docs/phases/PHASE-31-AUTHV2.md). When
// CHALK_AUTH_V2_REQUIRED is on (the default), a session belonging to a user
// who has NOT enrolled in auth v2 (password + confirmed TOTP) is rejected
// with 409 auth_v2_enrollment_required on every session-gated endpoint
// EXCEPT the small allowlist needed to complete enrollment. The client sees
// the 409 (or the auth_v2_enrolled=false field on /api/auth/me) and routes
// the user into the migration wizard.
//
