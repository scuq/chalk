...
> **Note, 9 August 2026 — chalk quotes now.** Phase 99 shipped a "quote" row
...
> twice as long as the conversation in it — is a real cost that 99 accepted
> knowingly, and if it shows up in practice, that is an argument *for*
> building this, not against it.
>
> One thing 99 settled that this document left open: `> ` is now a rendered
> construct, so a tie must not reuse it for anything. Full record:
> [PHASE-99-QUOTE.md](PHASE-99-QUOTE.md).
